...
- Report the correct module version in web service requests and the module information box.
...
- Italy as allowed country for _Closest Droppoint_ (CDP) service.

## 3.5.1

### Added

- The _Electronic Export Notification_ checkbox now stays in sync with the package's customs value while editing a shipment and stops auto-syncing once it is toggled manually.

### Fixed

- The _Electronic Export Notification_ checkbox is now pre-selected from the package's customs value instead of the whole order subtotal, so partial shipments below 1000 EUR are no longer pre-selected by mistake.
...
- NL, CY, CZ, EE, LT, LV as allowed country for _Closest Droppoint_ (CDP) service.

### Changed

- Remove HU as allowed country for _Closest Droppoint_ (CDP) service.

## 3.4.2

### Changed

- Make _Postal Delivery Duty Paid_ (PDDP) service mandatory for USA shipments under $800.

## 3.4.1

### Added

- France as allowed country for _Closest Droppoint_ (CDP) service.
